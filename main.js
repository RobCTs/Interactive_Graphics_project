...
function init() {
    ...
    const objLoader = new OBJLoader(), textureLoader = new THREE.TextureLoader(), texture = textureLoader.load('assets/nyra.png');
    objLoader.setPath('assets/').load('nyra.obj', (object) => {
...
function updatePersonPosition() {
    ...
    updateCameraPosition(); // Update camera position and zoom
}
...
function updateCameraPosition() {
    if (person) {
        ...
        camera.position.y = Math.max(person.position.y + 2, 1); // Ensure the camera stays above ground level
